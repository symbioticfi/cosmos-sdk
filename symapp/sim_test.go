...
// symappChainID hardcoded chainID for simulation
...
func TestFullAppSimulation(t *testing.T) {
	sims.Run(t, NewSymApp, setupStateFactory)
}

func setupStateFactory(app *SymApp) sims.SimStateFactory {
...
func TestAppImportExport(t *testing.T) {
	sims.Run(t, NewSymApp, setupStateFactory, func(t *testing.T, ti sims.TestInstance[*SymApp]) {
		app := ti.App
		...
		newTestInstance := sims.NewSimulationAppInstance(t, ti.Cfg, NewSymApp)
...
func TestAppStateDeterminism(t *testing.T) {
	...
	interBlockCachingAppFactory := func(logger log.Logger, db dbm.DB, traceStore io.Writer, loadLatest bool, appOpts servertypes.AppOptions, baseAppOptions ...func(*baseapp.BaseApp)) *SymApp {
		...
		return NewSymApp(logger, db, nil, true, appOpts, append(baseAppOptions, interBlockCacheOpt())...)
	}
	var mx sync.Mutex
	appHashResults := make(map[int64][][]byte)
	appSimLogger := make(map[int64][]simulation.LogWriter)
	captureAndCheckHash := func(t *testing.T, ti sims.TestInstance[*SymApp]) {
