...
// Profile with:
// /usr/local/go/bin/go test -benchmem -run=^$ cosmossdk.io/symapp -bench ^BenchmarkFullAppSimulation$ -Commit=true -cpuprofile cpu.out
func BenchmarkFullAppSimulation(b *testing.B) {
	b.ReportAllocs()

	config := simcli.NewConfigFromFlags()
	config.ChainID = sims.symappChainID
	...
	app := NewSymApp(logger, db, nil, true, appOptions, interBlockCacheOpt(), baseapp.SetChainID(sims.symappChainID))
