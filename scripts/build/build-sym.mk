...
BUILD_TARGETS := build-sym install

#? build: Build simapp
build-sym: BUILD_ARGS=-o $(BUILDDIR)/

#? build-linux-amd64: Build simapp for GOOS=linux GOARCH=amd64
build-sym-linux-amd64:
	GOOS=linux GOARCH=amd64 LEDGER_ENABLED=false $(MAKE) build

#? build-linux-arm64: Build simapp for GOOS=linux GOARCH=arm64
build-sym-linux-arm64:
	GOOS=linux GOARCH=arm64 LEDGER_ENABLED=false $(MAKE) build

$(BUILD_TARGETS): go.sum $(BUILDDIR)/
	cd ${CURRENT_DIR}/${SIMAPP} && go $(subst -sym,,$@) -mod=readonly $(BUILD_FLAGS) $(BUILD_ARGS) ./...
...
.PHONY: build-sym build-sym-linux-amd64 build-sym-linux-arm64 cosmovisor confix
