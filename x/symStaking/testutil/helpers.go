...
// commission
func (sh *Helper) CreateValidatorWithValPower(addr sdk.ValAddress, pk cryptotypes.PubKey, ok bool) math.Int {
	sh.createValidator(addr, pk, ok)
	return math.ZeroInt()
}

// CreateValidatorMsg returns a message used to create validator in this service.
func (sh *Helper) CreateValidatorMsg(addr sdk.ValAddress, pk cryptotypes.PubKey) *stakingtypes.MsgCreateValidator {
	address, err := sh.k.ValidatorAddressCodec().BytesToString(addr)
	require.NoError(sh.t, err)
	msg, err := stakingtypes.NewMsgCreateValidator(address, pk, stakingtypes.Description{}, sh.Commission)
	require.NoError(sh.t, err)
	return msg
}
...
func (sh *Helper) createValidator(addr sdk.ValAddress, pk cryptotypes.PubKey, ok bool) {
	address, err := sh.k.ValidatorAddressCodec().BytesToString(addr)
	require.NoError(sh.t, err)
	msg, err := stakingtypes.NewMsgCreateValidator(address, pk, stakingtypes.Description{Moniker: "TestValidator"}, sh.Commission)
	require.NoError(sh.t, err)
