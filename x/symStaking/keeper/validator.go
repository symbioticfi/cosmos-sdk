...
func (k Keeper) AddValidatorTokens(ctx context.Context, validator types.Validator,
	tokensToAdd math.Int,
) (valOut types.Validator, err error) {
	err = k.DeleteValidatorByPowerIndex(ctx, validator)
	if err != nil {
		return valOut, err
	}

	validator = validator.AddTokens(tokensToAdd)
...
func (k Keeper) SetValidatorTokens(ctx context.Context, validator types.Validator,
	tokens math.Int,
) (valOut types.Validator, err error) {
	err = k.DeleteValidatorByPowerIndex(ctx, validator)
	if err != nil {
	...
	if tokens.GT(validator.GetTokens()) {
		validator = validator.AddTokens(tokens.Sub(validator.GetTokens()))
	} else {
		validator = validator.RemoveTokens(validator.GetTokens().Sub(tokens))
	}
...
func (k Keeper) TotalBondedTokens(ctx context.Context) (math.Int, error) {
	lastTotalPower, err := k.LastTotalPower.Get(ctx)
	if err != nil {
		return math.ZeroInt(), err
	}
	return k.TokensFromConsensusPower(ctx, lastTotalPower.Int64()), nil
}
