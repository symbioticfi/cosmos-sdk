...
func ExportCmd(appExporter servertypes.AppExporter) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			appGenesis, err := symGenutiltypes.AppGenesisFromFile(config.GenesisFile())
			if err != nil {
				return err
			}
			...
			appGenesis.Consensus = symGenutiltypes.NewConsensusGenesis(exported.ConsensusParams, exported.Validators)
