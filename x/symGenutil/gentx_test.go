...
import (
	...
	symGenutiltestutil "github.com/cosmos/cosmos-sdk/x/symGenutil/testutil"
...
type GenTxTestSuite struct {
	...
	stakingKeeper  *symGenutiltestutil.MockStakingKeeper
...
func (suite *GenTxTestSuite) SetupTest() {
	...
	suite.encodingConfig = moduletestutil.MakeTestEncodingConfig(codectestutil.CodecOptions{}, symGenutil.AppModule{})
	...
	suite.stakingKeeper = symGenutiltestutil.NewMockStakingKeeper(ctrl)
	...
	suite.NoError(err)
	suite.msg1, err = stakingtypes.NewMsgCreateValidator(pk1Addr, pk1, desc, comm)
	suite.NoError(err)
	pk2Addr, err := valAc.BytesToString(pk2.Address())
	suite.NoError(err)
	suite.msg2, err = stakingtypes.NewMsgCreateValidator(pk2Addr, pk1, desc, comm)
	suite.NoError(err)
}
...
func (suite *GenTxTestSuite) TestSetGenTxsInAppGenesisState() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			tc.malleate()
			appGenesisState, err := symGenutil.SetGenTxsInAppGenesisState(cdc, txJSONEncoder, make(map[string]json.RawMessage), genTxs)
...
func (suite *GenTxTestSuite) TestValidateAccountInGenesis() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			tc.malleate()
			err = symGenutil.ValidateAccountInGenesis(
		...
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			if tc.expPass {
				...
				suite.Require().NotPanics(func() {
					_, err := symGenutil.DeliverGenTxs(
