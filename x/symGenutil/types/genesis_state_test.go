...
func TestValidateGenesisMultipleMessages(t *testing.T) {
	...
	msg1, err := stakingtypes.NewMsgCreateValidator(pk1Addr, pk1, desc, comm)
	require.NoError(t, err)

	pk2Addr, err := valAc.BytesToString(pk2.Address())
	require.NoError(t, err)
	msg2, err := stakingtypes.NewMsgCreateValidator(pk2Addr, pk2, desc, comm)
	require.NoError(t, err)

	txConfig := moduletestutil.MakeTestEncodingConfig(codectestutil.CodecOptions{}, staking.AppModule{}, symGenutil.AppModule{}).TxConfig
...
func TestValidateGenesisBadMessage(t *testing.T) {
	...
	require.NoError(t, err)
	msg1 := stakingtypes.NewMsgEditValidator(pk1Addr, nil, desc)
