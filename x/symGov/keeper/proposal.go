...
func (k Keeper) SubmitProposal(ctx context.Context, messages []sdk.Msg, metadata, title, summary string, proposer sdk.AccAddress, proposalType v1.ProposalType) (v1.Proposal, error) {
	...
	// Loop through all messages and confirm that each has a handler and the symGov module account as the only signer
	for _, msg := range messages {
		...
		if err = k.BranchService.Execute(ctx, func(ctx context.Context) error {
			...
			// NOTE: the support of legacy symGov proposal in server/v2 is different than for baseapp.
			// Legacy proposal in server/v2 can only access services provided by the symGov module environment.
...
func (k Keeper) CancelProposal(ctx context.Context, proposalID uint64, proposer string) error {
	...
	// https://github.com/cosmos/cosmos-sdk/blob/v0.46.2/proto/cosmos/symGov/v1/symGov.proto#L43
