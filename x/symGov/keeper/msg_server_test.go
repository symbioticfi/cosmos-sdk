...
func (suite *KeeperTestSuite) TestMsgSubmitProposal() {
	...
	cases := map[string]struct {
		...
		"invalid deposited coin": {
			...
			expErr:    true,
			expErrMsg: "deposited 100invalid, but symGov accepts only the following denom(s): [stake]: invalid deposit denom",
		},
		"invalid deposited coin (multiple)": {
			...
			expErr:    true,
			expErrMsg: "deposited 100invalid,100000stake, but symGov accepts only the following denom(s): [stake]: invalid deposit denom",
		},
		"all good": {
			preRun: func() (*v1.MsgSubmitProposal, error) {
				return v1.NewMsgSubmitProposal(
...
func (suite *KeeperTestSuite) TestMsgDeposit() {
	...
	cases := map[string]struct {
		...
		"invalid deposited coin ": {
			...
			expErr:    true,
			expErrMsg: "deposited 1000ibc/badcoin, but symGov accepts only the following denom(s): [stake]",
		},
		"invalid deposited coin (multiple)": {
			preRun: func() uint64 {
				return pID
			...
			expErr:    true,
			expErrMsg: "deposited 1000ibc/badcoin,10000000stake, but symGov accepts only the following denom(s): [stake]: invalid deposit denom",
		},
		"all good": {
			preRun: func() uint64 {
				return pID
