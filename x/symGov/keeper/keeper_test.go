...
import (
	...
	govtestutil "cosmossdk.io/x/symGov/testutil"
...
func (suite *KeeperTestSuite) reset() {
	...
	// Populate the symGov account with some coins, as the TestProposal we have
	// is a MsgSend from the symGov account.
...
func TestGetGovGovernanceAndModuleAccountAddress(t *testing.T) {
	govKeeper, mocks, _, ctx := setupGovKeeper(t)
	authKeeper := mocks.acctKeeper
	mAcc := authKeeper.GetModuleAccount(ctx, "symGov")
	require.Equal(t, mAcc, govKeeper.GetGovernanceAccount(ctx))
	mAddr := authKeeper.GetModuleAddress("symGov")
