...
func (k Keeper) EndBlocker(ctx context.Context) error {
	...
	for _, prop := range inactiveProps {
		...
		if err != nil {
			// if the proposal has an encoding error, this means it cannot be processed by x/symGov
	...
	for _, prop := range activeProps {
		...
		if err != nil {
			...
			// as it could happen due to a governance mistake (governance has let a proposal pass that sends symGov funds that were from proposal deposits)
...
func safeExecuteHandler(ctx context.Context, msg sdk.Msg, router router.Service) (res protoiface.MessageV1, err error) {
	defer func() {
		if r := recover(); r != nil {
			err = fmt.Errorf("handling x/symGov proposal msg [%s] PANICKED: %v", msg, r)
		}
	}()
...
// failUnsupportedProposal fails a proposal that cannot be processed by symGov
func failUnsupportedProposal(
	...
	proposal.Status = v1.StatusFailed
	proposal.FailedReason = fmt.Sprintf("proposal failed because it cannot be processed by symGov: %s", errMsg)
