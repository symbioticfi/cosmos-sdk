...
func TestDepositAmount(t *testing.T) {
	testcases := []struct {
		...
		},
		{
			...
			minDepositRatio: "0.001",
			err:             "deposited 10000euro, but symGov accepts only the following denom(s): [stake zcoin]: invalid deposit denom",
		},
		{
			name:            "mix containing bad and good denom",
			deposit:         sdk.NewCoins(sdk.NewInt64Coin("stake", 10000), sdk.NewInt64Coin("euro", 10000)),
			minDepositRatio: "0.001",
			err:             "deposited 10000euro,10000stake, but symGov accepts only the following denom(s): [stake zcoin]: invalid deposit denom",
		},
		{
