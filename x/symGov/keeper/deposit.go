...
// SetDeposit sets a Deposit to the symGov store
...
func (k Keeper) validateDepositDenom(params v1.Params, depositAmount sdk.Coins) error {
	...
	for _, coin := range depositAmount {
		if _, ok := acceptedDenoms[coin.Denom]; !ok {
			return errors.Wrapf(types.ErrInvalidDepositDenom, "deposited %s, but symGov accepts only the following denom(s): %v", depositAmount, denoms)
		}
	}
