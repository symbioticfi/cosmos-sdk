...
proposal will be removed from state and the deposit will be burned (see x/symGov `EndBlocker`).
...
[ADR-037](https://github.com/cosmos/cosmos-sdk/blob/main/docs/architecture/adr-037-symGov-split-vote.md) introduces the weighted vote feature which allows a staker to split their votes into several voting options. For example, it could use 70% of its voting power to vote Yes and 30% of its voting power to vote No.
...
```protobuf reference
https://github.com/cosmos/cosmos-sdk/blob/v0.47.0-rc1/proto/cosmos/symGov/v1beta1/symGov.proto#L34-L47
```

```protobuf reference
https://github.com/cosmos/cosmos-sdk/blob/v0.47.0-rc1/proto/cosmos/symGov/v1beta1/symGov.proto#L181-L201
```

For a weighted vote to be valid, the `options` field must not contain duplicate vote options, and the sum of weights of all options must be equal to 1.

The maximum number of weighted vote options can be limited by the developer via a config parameter, named `MaxVoteOptionsLen`, which gets passed into the symGov keeper.
...
```protobuf reference
https://github.com/cosmos/cosmos-sdk/blob/v0.47.0-rc1/proto/cosmos/symGov/v1/symGov.proto#L51-L99
```
...
passed into the symGov keeper as a config. The default maximum length are: for the title 255 characters, for the metadata 255 characters and for summary 10200 characters (40 times the one of the title).
...
```protobuf reference
https://github.com/cosmos/cosmos-sdk/blob/v0.47.0-rc1/proto/cosmos/symGov/v1/symGov.proto#L152-L162
```

#### VotingParams

```protobuf reference
https://github.com/cosmos/cosmos-sdk/blob/v0.47.0-rc1/proto/cosmos/symGov/v1/symGov.proto#L164-L168
```

#### TallyParams

```protobuf reference
https://github.com/cosmos/cosmos-sdk/blob/v0.47.0-rc1/proto/cosmos/symGov/v1/symGov.proto#L170-L182
```
...
```protobuf reference
https://github.com/cosmos/cosmos-sdk/blob/v0.47.0-rc1/proto/cosmos/symGov/v1/symGov.proto#L38-L49
```
...
These proposals are defined by their types and handled by handlers that are registered in the symGov v1beta1 router.
...
```protobuf reference
https://github.com/cosmos/cosmos-sdk/blob/v0.47.0-rc1/proto/cosmos/symGov/v1/tx.proto#L42-L69
```
...
have one signer, namely the symGov module account. And finally, the metadata length
must not be larger than the `maxMetadataLen` config passed into the symGov keeper.
...
```protobuf reference
https://github.com/cosmos/cosmos-sdk/blob/v0.47.0-rc1/proto/cosmos/symGov/v1/tx.proto#L134-L147
```
...
```protobuf reference
https://github.com/cosmos/cosmos-sdk/blob/v0.47.0-rc1/proto/cosmos/symGov/v1/tx.proto#L92-L108
```
...
A user can query and interact with the `symGov` module using the CLI.

#### Query

The `query` commands allow users to query `symGov` state.

```bash
simd query symGov --help
```
...
```bash
simd query symGov deposit [proposal-id] [depositer-addr] [flags]
```

Example:

```bash
simd query symGov deposit 1 cosmos1..
```
...
```bash
simd query symGov deposits [proposal-id] [flags]
```

Example:

```bash
simd query symGov deposits 1
```
...
The `params` command allows users to query all parameters for the `symGov` module.

```bash
simd query symGov params [flags]
```

Example:

```bash
simd query symGov params
```
...
```bash
simd query symGov proposal [proposal-id] [flags]
```

Example:

```bash
simd query symGov proposal 1
```
...
```bash
simd query symGov proposals [flags]
```

Example:

```bash
simd query symGov proposals
```
...
```bash
simd query symGov proposer [proposal-id] [flags]
```

Example:

```bash
simd query symGov proposer 1
```
...
```bash
simd query symGov tally [proposal-id] [flags]
```

Example:

```bash
simd query symGov tally 1
```
...
```bash
simd query symGov vote [proposal-id] [voter-addr] [flags]
```

Example:

```bash
simd query symGov vote 1 cosmos1..
```
...
```bash
simd query symGov votes [proposal-id] [flags]
```

Example:

```bash
simd query symGov votes 1
```
...
The `tx` commands allow users to interact with the `symGov` module.
...
```bash
simd tx symGov deposit [proposal-id] [deposit] [flags]
```

Example:

```bash
simd tx symGov deposit 1 10000000stake --from cosmos1..
```
...
```bash
simd tx symGov submit-proposal [path-to-proposal-json] [flags]
```

Example:

```bash
simd tx symGov submit-proposal /path/to/proposal.json --from cosmos1..
```
...
```json
{
  "messages": [
    {
      "@type": "/cosmos.bank.v1beta1.MsgSend",
      "from_address": "cosmos1...", // The symGov module module address
      "to_address": "cosmos1...",
...
```bash
simd tx symGov submit-legacy-proposal [command] [flags]
```

Example:

```bash
simd tx symGov submit-legacy-proposal --title="Test Proposal" --description="testing" --type="Text" --deposit="100000000stake" --from cosmos1..
```

Example (`param-change`):

```bash
simd tx symGov submit-legacy-proposal param-change proposal.json --from cosmos1..
```
...
```bash
simd tx symGov cancel-proposal [proposal-id] [flags]
```

Example:

```bash
simd tx symGov cancel-proposal 1 --from cosmos1...
```
...
```bash
simd tx symGov vote [command] [flags]
```

Example:

```bash
simd tx symGov vote 1 yes --from cosmos1..
```
...
```bash
simd tx symGov weighted-vote [proposal-id] [weighted-options] [flags]
```

Example:

```bash
simd tx symGov weighted-vote 1 yes=0.5,no=0.5 --from cosmos1..
```

### gRPC

A user can query the `symGov` module using gRPC endpoints.
...
```bash
cosmos.symGov.v1beta1.Query/Proposal
```
...
```bash
{
  "proposal": {
    "proposalId": "1",
    "content": {"@type":"/cosmos.symGov.v1beta1.TextProposal","description":"testing, testing, 1, 2, 3","title":"Test Proposal"},
...
```bash
cosmos.symGov.v1.Query/Proposal
```
...
```bash
cosmos.symGov.v1beta1.Query/Proposals
```
...
```bash
cosmos.symGov.v1.Query/Proposals
```
...
```bash
cosmos.symGov.v1beta1.Query/Vote
```
...
```bash
cosmos.symGov.v1beta1.Query/Votes
```
...
The `Params` endpoint allows users to query all parameters for the `symGov` module.

Using legacy v1beta1:

```bash
cosmos.symGov.v1beta1.Query/Params
```
...
```bash
cosmos.symGov.v1beta1.Query/Deposit
```
...
```bash
cosmos.symGov.v1.Query/Deposit
```
...
```bash
cosmos.symGov.v1beta1.Query/Deposits
```
...
```bash
cosmos.symGov.v1.Query/Deposits
```
...
```bash
cosmos.symGov.v1beta1.Query/TallyResult
```
...
```bash
cosmos.symGov.v1.Query/TallyResult
```
...
A user can query the `symGov` module using REST endpoints.
...
```bash
/cosmos/symGov/v1beta1/proposals/{proposal_id}
```

Example:

```bash
curl localhost:1317/cosmos/symGov/v1beta1/proposals/1
```
...
```bash
/cosmos/symGov/v1/proposals/{proposal_id}
```

Example:

```bash
curl localhost:1317/cosmos/symGov/v1/proposals/1
```
...
```bash
/cosmos/symGov/v1beta1/proposals
```

Example:

```bash
curl localhost:1317/cosmos/symGov/v1beta1/proposals
```
...
```bash
/cosmos/symGov/v1/proposals
```

Example:

```bash
curl localhost:1317/cosmos/symGov/v1/proposals
```
...
```bash
/cosmos/symGov/v1beta1/proposals/{proposal_id}/votes/{voter}
```

Example:

```bash
curl localhost:1317/cosmos/symGov/v1beta1/proposals/1/votes/cosmos1..
```
...
```bash
/cosmos/symGov/v1/proposals/{proposal_id}/votes/{voter}
```

Example:

```bash
curl localhost:1317/cosmos/symGov/v1/proposals/1/votes/cosmos1..
```
...
```bash
/cosmos/symGov/v1beta1/proposals/{proposal_id}/votes
```

Example:

```bash
curl localhost:1317/cosmos/symGov/v1beta1/proposals/1/votes
```
...
```bash
/cosmos/symGov/v1/proposals/{proposal_id}/votes
```

Example:

```bash
curl localhost:1317/cosmos/symGov/v1/proposals/1/votes
```
...
```bash
/cosmos/symGov/v1beta1/params/{params_type}
```

Example:

```bash
curl localhost:1317/cosmos/symGov/v1beta1/params/voting
```
...
```bash
/cosmos/symGov/v1/params/{params_type}
```

Example:

```bash
curl localhost:1317/cosmos/symGov/v1/params/voting
```
...
```bash
/cosmos/symGov/v1beta1/proposals/{proposal_id}/deposits/{depositor}
```

Example:

```bash
curl localhost:1317/cosmos/symGov/v1beta1/proposals/1/deposits/cosmos1..
```
...
```bash
/cosmos/symGov/v1/proposals/{proposal_id}/deposits/{depositor}
```

Example:

```bash
curl localhost:1317/cosmos/symGov/v1/proposals/1/deposits/cosmos1..
```
...
```bash
/cosmos/symGov/v1beta1/proposals/{proposal_id}/deposits
```

Example:

```bash
curl localhost:1317/cosmos/symGov/v1beta1/proposals/1/deposits
```
...
```bash
/cosmos/symGov/v1/proposals/{proposal_id}/deposits
```

Example:

```bash
curl localhost:1317/cosmos/symGov/v1/proposals/1/deposits
```
...
```bash
/cosmos/symGov/v1beta1/proposals/{proposal_id}/tally
```

Example:

```bash
curl localhost:1317/cosmos/symGov/v1beta1/proposals/1/tally
```
...
```bash
/cosmos/symGov/v1/proposals/{proposal_id}/tally
```

Example:

```bash
curl localhost:1317/cosmos/symGov/v1/proposals/1/tally
```
