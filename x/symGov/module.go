...
import (
	...
	govclient "cosmossdk.io/x/symGov/client"
	"cosmossdk.io/x/symGov/client/cli"
	"cosmossdk.io/x/symGov/keeper"
	"cosmossdk.io/x/symGov/simulation"
...
// AppModule implements an application module for the symGov module.
...
// Name returns the symGov module's name.
// Deprecated: kept for legacy reasons.
func (AppModule) Name() string {
	return govtypes.ModuleName
}

// RegisterLegacyAminoCodec registers the symGov module's types for the given codec.
...
// RegisterGRPCGatewayRoutes registers the gRPC Gateway routes for the symGov module.
...
// GetTxCmd returns the root tx command for the symGov module.
...
func (am AppModule) RegisterMigrations(mr appmodule.MigrationRegistrar) error {
	...
	if err := mr.Register(govtypes.ModuleName, 1, m.Migrate1to2); err != nil {
		return fmt.Errorf("failed to migrate x/symGov from version 1 to 2: %w", err)
	}

	if err := mr.Register(govtypes.ModuleName, 2, m.Migrate2to3); err != nil {
		return fmt.Errorf("failed to migrate x/symGov from version 2 to 3: %w", err)
	}

	if err := mr.Register(govtypes.ModuleName, 3, m.Migrate3to4); err != nil {
		return fmt.Errorf("failed to migrate x/symGov from version 3 to 4: %w", err)
	}

	if err := mr.Register(govtypes.ModuleName, 4, m.Migrate4to5); err != nil {
		return fmt.Errorf("failed to migrate x/symGov from version 4 to 5: %w", err)
	}

	if err := mr.Register(govtypes.ModuleName, 5, m.Migrate5to6); err != nil {
		return fmt.Errorf("failed to migrate x/symGov from version 5 to 6: %w", err)
	}

	return nil
}

// DefaultGenesis returns default genesis state as raw bytes for the symGov module.
func (am AppModule) DefaultGenesis() json.RawMessage {
	return am.cdc.MustMarshalJSON(v1.DefaultGenesisState())
}

// ValidateGenesis performs genesis state validation for the symGov module.
...
// InitGenesis performs genesis initialization for the symGov module.
...
// ExportGenesis returns the exported genesis state as raw bytes for the symGov module.
...
// EndBlock returns the end blocker for the symGov module.
...
// GenerateGenesisState creates a randomized GenState of the symGov module.
func (AppModule) GenerateGenesisState(simState *module.SimulationState) {
	simulation.RandomizedGenState(simState)
}

// ProposalContents returns all the symGov content functions used to
...
// ProposalMsgs returns all the symGov msgs used to simulate governance proposals.
func (AppModule) ProposalMsgs(simState module.SimulationState) []simtypes.WeightedProposalMsg {
	return simulation.ProposalMsgs()
}

// RegisterStoreDecoder registers a decoder for symGov module's types
func (am AppModule) RegisterStoreDecoder(sdr simtypes.StoreDecoderRegistry) {
	sdr[govtypes.StoreKey] = simtypes.NewStoreDecoderFuncFromCollectionsSchema(am.keeper.Schema)
}

// WeightedOperations returns the all the symGov module operations with their respective weights.
