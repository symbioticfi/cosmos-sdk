...
import (
	...
	govutils "cosmossdk.io/x/symGov/client/utils"
...
Example:
$ %s tx symGov submit-proposal path/to/proposal.json
...
Example:
$ %s tx symGov submit-legacy-proposal --proposal="path/to/proposal.json" --from mykey
...
$ %s tx symGov submit-legacy-proposal --title="Test Proposal" --description="My awesome proposal" --type="Text" --deposit="10test" --from mykey
`,
...
find the proposal-id by running "%s query symGov proposals".

Example:
$ %s tx symGov weighted-vote 1 yes=0.6,no=0.3,abstain=0.05,no-with-veto=0.05 --from mykey
`,
				version.AppName, version.AppName,
			),
		),
